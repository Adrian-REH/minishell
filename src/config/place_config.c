...
t_exec	*place_exec(t_handler *a, t_exec *exec, int *i_exec)
{
	...
	{
		b->next_exec = \
		ft_realloc(b->next_exec, sizeof(t_exec) * (b->len_exec_next), sizeof(t_exec) * (b->len_exec_next + 1));
		exec = b->next_exec;
	...
	{
		b->prev_exec = \
		ft_realloc(b->prev_exec, sizeof(t_exec) * (b->len_exec_prev) , sizeof(t_exec) * (b->len_exec_prev + 1));
...
t_handler	*place_uniq_cmd(t_handler *a, int i)
{
	...
	&& a->state[1] == 14 && a->state[2] == 0)
		if ((tmp = do_exec(a->info->tokens[i - 1], a->env)))
